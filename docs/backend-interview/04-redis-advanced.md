...
if (item.expire < now()) {
    new Thread(() -> {          // 后台异步刷新
        if (redis.setnx(lockKey, "1", 3)) {
            item.data = db.query(key);
            item.expire = now() + 3600;
            redis.set(key, item);
            redis.del(lockKey);
        }
    }).start();
}
return item.data;  // 未过期的旧数据
```
...
// 基础过期 + 随机偏移，避免同时过期
int baseTTL = 3600;
int random = ThreadLocalRandom.current().nextInt(600); // 0-600s 随机
...
if (lock.tryLock(3, 30, TimeUnit.SECONDS)) {  // 等待 3s，租期 30s
    try {
        // 业务逻辑
    } finally {
        lock.unlock();
...
while (true) {
    long now = System.currentTimeMillis();
    Set<String> tasks = redis.zrangeByScore("delay:queue", 0, now, 0, 100);
    for (String task : tasks) {
        if (redis.zrem("delay:queue", task) > 0) {  // 移除成功才执行
            execute(task);
        }
    }
    Thread.sleep(100);
...
```java
// 1. 生产者发送到普通 Stream
redis.xadd("stream:orders", Map.of("orderId", "123", "delay", "30000"));

// 2. 消费者读取 p1（正常消费组）
// 3. 检查消息是否需要延迟处理：通过 XINFO PENDING 检测消息是否到期
// 4. 未到期消息使用 XCLAIM 延迟处理
```
