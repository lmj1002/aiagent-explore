...
>
> A:
> - **B-Tree vs B+Tree**：B-Tree 每个节点都存数据，导致每个节点能存的 key 数少，树更高，IO 次数更多。B+Tree 数据只存叶子节点，非叶子节点更"扁平"，磁盘 IO 更少。B+Tree 叶子节点有双向链表支持范围查询。
> - **红黑树**：二叉平衡树，高度 logN，百万级数据需要约 20 层，即 20 次磁盘 IO，太慢。
> - **哈希表**：仅支持等值查询(=、IN)，不支持范围查询(>、<、BETWEEN)和排序，有哈希冲突问题。
...
- **Buffer Pool 命中率过低**：`show engine innodb status` 查看 `Buffer pool hit rate`。若低于 95%，说明内存不足或 SQL 扫描数据量过大。
  ```sql
  -- 查看缓冲池命中率
  SHOW GLOBAL STATUS LIKE 'Innodb_buffer_pool_reads';       -- 从磁盘读取次数
  SHOW GLOBAL STATUS LIKE 'Innodb_buffer_pool_read_requests'; -- 总读取次数
  -- 命中率 = (read_requests - reads) / read_requests
  ```
- **实例重启后 Buffer Pool 预热问题**：5.6+ 支持 `innodb_buffer_pool_dump_at_shutdown=ON` 和 `innodb_buffer_pool_load_at_startup=ON`，可加速预热。
- **大事务导致 Buffer Pool 脏页刷盘慢**：监控 `Innodb_buffer_pool_dirty_pages`，通过 `innodb_io_capacity` 和 `innodb_io_capacity_max` 控制刷盘速度。
